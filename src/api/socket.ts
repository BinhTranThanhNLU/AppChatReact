...
export const connectSocket = (onOpen?: () => void) => {
    ...
    socket.onmessage = (e) => {
        ...
        // 1.1Xử lý kết quả ĐĂNG KÝ
        if (res.event === "REGISTER") {
            if (res.status === "success") {
                console.log("Đăng ký thành công");
                window.dispatchEvent(new CustomEvent("register-success", {detail: "Đăng ký tài khoản thành công!"}));
            } else {
                console.error("Đăng ký thất bại:", res.mes);
                window.dispatchEvent(new CustomEvent("register-error", {detail: res.mes || "Tên tài khoản đã tồn tại!"}));
            }
            return;
        }
